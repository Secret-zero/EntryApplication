    ...
    <div class='btm'>
      ...
      <div class='input-box'>
        ...
        <div class='val-box' v-if='dutyList.length&&searchValue&&showBox1'>
          <ul>
            <li v-for='(item, index) in dutyList' :key='index' @click='searchValue = item,dutyList = [], showBox1 = false'>{{item}}</li>
          </ul>
        </div>
      </div>
      ...
      <mt-button type="default" class='mtbutton' @click='toResume'>进入简历填写</mt-button>
    </div>
...
export default {
  ...
  methods: {
    ...
    toResume() {
      ...
      this.$router.push({path: '/ctalog', query: {type: this.FJobType==0?3:this.FJobType}})
    }
  }
 }
</script>