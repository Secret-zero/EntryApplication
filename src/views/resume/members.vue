<template>
  <div class="wrap">
    <div class="item" v-for="(item, index) in qsgx" :key="index" :class='index>0 ? "mt20" : ""'>
...
export default {
  ...
  methods: {
    ...
    confirm() {
			console.info(this.qsgx);
			for(var i=0;i<this.qsgx.length;i++){
				if(this.qsgx[i].Fname==""){
					this.$toast({message:'请填写家庭主要成员'+(i+1)+'姓名',duration:1500});
					return;
				}
				if(this.qsgx[i].Fguanxi==""){
					this.$toast({message:'请填写家庭主要成员'+(i+1)+'关系',duration:1500});
					return;
				}
				if(this.qsgx[i].FTel==""){
					this.$toast({message:'请填写家庭主要成员'+(i+1)+'联系电话',duration:1500});
					return;
				}
			}
      // let data = this.qsgx.filter((item) => {
