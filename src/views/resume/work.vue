<template>
  <div class='wrap'>
    <div class='item' v-for='(item, index) in workList' :key='index' :class='index>0 ? "mt20" : ""'>
      ...
      <div class='relations'>
        <ul>
          ...
          <li>
            ...
            <div class='content'>
              <input type="text" placeholder="请填写" v-model='item.FWorkCompany' />
            </div>
          </li>
          <li>
            <div class='name'>所任职务</div>
            <div class='content'>
              <input type="text" placeholder="请填写" v-model='item.FdutyName' />
            </div>
          </li>
          <li>
            <div class='name'>离职原因</div>
            <div class='content'>
              <input type="text" placeholder="请填写" v-model='item.FleaveBecuase' />
            </div>
          </li>
...
export default {
  ...
  mounted() {
		...
		if(parseInt(this.$route.query.type)===1){
			for(let i=this.workList.length;i<this.defaultNumber1;i++) this.add();//默认两个
  ...
  methods: {
    confirm() {
			for(var i=0;i<this.workList.length;i++){
				if(this.workList[i].Ffromdate===""){
					this.$toast({message:'请选择工作经历'+(i+1)+'开始年月',duration:1500});
					return;
				}
				if(this.workList[i].Ftodate===""){
					this.$toast({message:'请选择工作经历'+(i+1)+'结束年月',duration:1500});
					return;
				}
				if(this.workList[i].FWorkCompany===""){
					this.$toast({message:'请填写工作经历'+(i+1)+'工作单位',duration:1500});
					return;
				}
			}
      ...
      // let data = this.workList.filter((item) => {
    ...
    popupBtn(value) {
      ...
      if (value === 'confirm') {
        ...
        _date = date.getFullYear() + '-' + (date.getMonth() + 1 < 10 ? '0' + (date.getMonth()+ 1) : date.getMonth() + 1) + '-' + (date.getDate() < 10 ? '0' + date.getDate() : date.getDate())
        this.$set(this.workList[this.index], 'Ffromdate', _date)
      } else {
				let date = new Date(this.endDate)
        _date = date.getFullYear() + '-' + (date.getMonth() + 1 < 10 ? '0' + (date.getMonth()+ 1) : date.getMonth() + 1) + '-' + (date.getDate() < 10 ? '0' + date.getDate() : date.getDate())
				if(this.workList[this.index].Ffromdate>_date&&this.workList[this.index].Ffromdate!=''){
