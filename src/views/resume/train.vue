<template>
  <div class="wrap">
    <div class="item" v-for="(item, index) in studyList" :key="index" :class="index> 0 ?'mt20':''">
      ...
      <div class="relations">
        <ul>
          ...
          <li>
            ...
            <div class="li-right v-flex" @click="openPickdate(index, 'dateTimePopup')">
              <input type="text" placeholder="请选择" readonly v-model="item.Ffromdate">
              <i class="iconfont icon-iconfontyoujiantou"></i>
            </div>
          </li>
          <li>
            <div class="name color1">至年月</div>
            <div class="li-right v-flex" @click="openPickdate(index, 'dateTimePopup2')">
              <input type="text" placeholder="请选择" readonly v-model="item.Ftodate">
              <i class="iconfont icon-iconfontyoujiantou"></i>
            </div>
          </li>
          <li>
          ...
          <li>
            ...
            <div class="li-right v-flex" @click="popupVisible = true, study_index = index">
              <input type="text" placeholder="请选择" readonly v-model="item.Fbiye">
              <i class="iconfont icon-iconfontyoujiantou"></i>
            </div>
          </li>
        </ul>
...
<script>
import { Button, Popup, Picker, DatetimePicker } from "mint-ui";
export default {
  data() {
    return {
      endDate: "2000-01-01",
    ...
    if(this.jyxl) {
      this.studyList = this.jyxl
		...
		if(parseInt(this.$route.query.type)===1){
			for(let i=this.studyList.length;i<this.defaultNumber1;i++) this.add();//默认两个
		}else if(parseInt(this.$route.query.type)===2){
			for(let i=this.studyList.length;i<this.defaultNumber2;i++) this.add();//默认三个
		}else{
			for(let i=this.studyList.length;i<this.defaultNumber3;i++) this.add();//默认三个
		}
  },
  methods: {
    confirm() {
      // let data = this.studyList.filter((item) => {
      //   return item.status === true
      // })
			// if (!data.length) return this.$toast('请至少选择一项资料进行上传');
			for(var i=0;i<this.studyList.length;i++){
				if(this.studyList[i].Fdengji===""){
					this.$toast({message:'请选择学习培训经历'+(i+1)+'学历',duration:1500});
					return;
				}
				if(this.studyList[i].Ffromdate===""){
					this.$toast({message:'请选择学习培训经历'+(i+1)+'开始年月',duration:1500});
					return;
				}
				if(this.studyList[i].Ftodate===""){
					this.$toast({message:'请选择学习培训经历'+(i+1)+'结束年月',duration:1500});
					return;
				}
				if(this.studyList[i].Fxuexiao===""){
					this.$toast({message:'请选择学习培训经历'+(i+1)+'学校及培训单位',duration:1500});
					return;
				}
				if(this.studyList[i].Fzhuanye===""){
					this.$toast({message:'请选择学习培训经历'+(i+1)+'专业',duration:1500});
					return;
				}
				if(this.studyList[i].Fbiye===""){
					this.$toast({message:'请选择学习培训经历'+(i+1)+'是否毕业/是否领证',duration:1500});
					return;
				}
			}
    ...
    popupBtn(value) {
      ...
      if (value === "cancel") {
        ...
        this.$set(
          this.studyList[this.study_index],
          "Fdengji",
          this.education
        );
