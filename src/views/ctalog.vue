<template>
  <div class='wrap'>
    <div class='list'>
      <div class='button' @click='toUrl("/infomation")' :class="main? 'backBtnColor' : 'bg'">
        基本信息
        <i class='iconfont icon-gou'></i>
      </div>
      <div class='button' @click='toUrl("/members")' :class="qsgx? 'backBtnColor' : 'bg'">
        家庭主要成员
        <i v-if='qsgx' class='iconfont icon-gou'></i>
      </div>
      <div class='button'  @click='toUrl("/train")' :class="jyxl? 'backBtnColor' : 'bg'">
        学习及培训经历
      ...
      <div class='button' @click='toUrl("/work")' :class="gzjl? 'backBtnColor' : 'bg'">
        主要工作经历
      ...
      <div class='button' @click='toUrl("/other")' :class="main2? 'backBtnColor' : 'bg'">
        其他信息
        <i class='iconfont icon-gou'></i>
      </div>
    </div>
...
export default {
  ...
  mounted() {
    ...
    if (this.getSession('main') && this.getSession('main').Fname) {
      ...
      this.form.jsonStrMain = [Object.assign(this.getSession('main2')==null?"{}":this.getSession('main2'), this.getSession('main'))]
    }
  ...
  methods: {
    ...
    async confirm() {
			if(this.form.jsonStrMain==''||this.form.jsonStrDetail1==''||this.form.jsonStrDetail2==''||this.form.jsonStrDetail3==''){
